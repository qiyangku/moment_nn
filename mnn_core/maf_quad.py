...
class MomentActivation():
    def __init__(self):
        '''Moment activation function; quadratic correction'''
        self.ds1 = Dawson1()
        self.ds2 = Dawson2()
        self.L = 0.05
        self.Tref = 5.0
    ...
    def chi2(self, ubar, sbar, cut_off=10, use_cache = True):
        '''Calculates the quadratic response coefficient of output firing rate given the mean & std of input firing rate'''
        ...
        gub =  self.ds1.dawson1(ub)
        glb =  self.ds1.dawson1(lb)  

        X2[indx2] = (4/self.L/np.sqrt(self.L))*self.u[indx2]*self.X[indx2]*(gub-glb) + \
            -4/self.L/self.L*self.u[indx2]*self.u[indx2]/self.s[indx2]*(ub*gub-lb*glb)
        ...
        X2[indx3] = 0.0
        # !!!!!!!!!!!!! UNDER CONSTRUCTION        
        #X2[indx4] = np.sqrt(2/self.L)/np.sqrt(self.Tref - 1/self.L*np.log(1-1/ubar[indx4]))/np.sqrt(2*ubar[indx4]-1)
        # !!!!!!!!!!!!
        self.X2 = X2
        return X2
