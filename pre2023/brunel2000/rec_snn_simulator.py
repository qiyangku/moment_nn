...
class SNNInputGenerator():
    ...
    def ind_poisson(self, dt):
        #dt = simulation time step (ms)
        p = self.uext*dt # requires p<1        
        input_current = self.we*(np.random.rand(self.N,1) < p)
...
class InteNFireRNN():
    ...
    def run(self, T, s = None, ntrials = None, record_v = False, show_message = False):
        ...
        #v, tref, is_spike
        cache_spk = np.zeros((self.num_neurons, delay_steps+1 ))
        ...
        for i in range(num_timesteps):
            ...
            spk_delayed = cache_spk[:,-1].reshape(-1,1)
            
            # update cache
            cache_spk = np.roll(cache_spk,1,axis = 1)
            cache_spk[:,0] = is_spike.ravel()
            
            #!!! spike input: independent Poisson
            input_current = self.input_gen.ind_poisson(self.dt)
